...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    const directory = (() => {
      ...
      if (process.env.OPENCODE_WORKING_DIR) {
        try {
          process.chdir(process.env.OPENCODE_WORKING_DIR)
          return process.cwd()
        } catch {
          UI.error("Failed to change directory to " + process.env.OPENCODE_WORKING_DIR)
          process.exit(1)
        }
      }
      return undefined
